...
void VulkanManager::CreateVertexBufer(std::vector<Vertex>& vertices, VkBuffer& buffer, VkDeviceMemory& memory, VkCommandPool& commandPool)
{
	...
	CopyBuffer(stagingBuffer, buffer, commandPool, bufferSize);
...
void VulkanManager::CreateIndexBuffer(std::vector<uint32_t>& indices, VkBuffer& buffer, VkDeviceMemory& memory,
	VkCommandPool& commandPool)
{
	...
	// VK_BUFFER_USAGE_TRANSFER_SRC_BIT - Use this buffer as the source in transferring memory.
	VulkanManager::GetVulkanManager().CreateBuffer(bufferSize, VK_BUFFER_USAGE_TRANSFER_SRC_BIT, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT, stagingBuffer, stagingBufferMemory);
...
void VulkanManager::CreateUniformBuffer(std::vector<VkBuffer>& buffers, std::vector<VkDeviceMemory>& deviceMemories)
{
	VkDeviceSize bufferSize = sizeof(UniformBufferObject);
	{
		buffers.resize(VulkanManager::GetVulkanManager().GetSwapChainImages().size());
		deviceMemories.resize(VulkanManager::GetVulkanManager().GetSwapChainImages().size());
	}
